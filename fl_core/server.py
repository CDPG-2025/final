    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in self.test_loader:
                if len(batch) == 3:
                    inputs, labels, sources = batch
                    sources = sources.numpy()
                else:
                    inputs, labels = batch
                    sources = np.zeros(len(labels)) # Default source 0
        ...
        source_names = {0: 'CIC-IDS-2017', 1: 'TON_IoT', 2: 'UNSW-NB15'}
        
        for s in unique_sources:
            idx = all_sources == s
            if np.sum(idx) > 0:
                s_labels = all_labels[idx]
                s_preds = all_preds[idx]
                s_acc = 100 * np.sum(s_labels == s_preds) / len(s_labels)
                s_f1 = f1_score(s_labels, s_preds, average='weighted')
                dataset_metrics[source_names.get(int(s), f'Source {s}')] = {'acc': s_acc, 'f1': s_f1}
        
        return accuracy, avg_loss, f1, precision, recall, dataset_metrics
